...
const studentSchema = z
  ...
  .refine((d) => d.billingType !== "MONTHLY" || (d.monthlyFee ?? 0) > 0, {
    message: "Mensalidade é obrigatória no plano mensal",
    path: ["monthlyFee"],
  })
  .refine(
    (d) =>
      d.billingType !== "HOURLY" || (d.hourlyRate != null && d.hourlyRate > 0),
    {
...
function GuardianPickerModal({
  ...
  const guardians = (guardiansData?.data ?? []).filter(
            ...
            {guardians.length === 0 ? (
              <p className="px-4 py-6 text-sm text-muted-foreground text-center">
                {search
                  ...
                  : "Todos os responsáveis já estão vinculados."}
              </p>
            ) : (
              guardians.map((g) => (
                <button
  ...
  const {
    ...
    defaultValues: student
      ? {
          ...
          guardianIds: (student.guardians ?? []).map(
            (g: { guardian: { id: string } }) => g.guardian.id,
          ),
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 w-full max-w-4xl mx-auto px-2 py-8 sm:px-6 sm:py-10">
            ...
            {guardianIds.length === 0 ? (
              <div className="text-center py-8 glass-panel bg-slate-900/30 rounded-xl border border-dashed border-white/10">
                ...
                <p className="text-xs text-slate-600 mt-1">
                  Cadastre responsáveis em{" "}
                  <strong className="text-slate-500">Responsáveis</strong> e
                  vincule aqui.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {guardianIds.map((gid) => {
                  const found =
                    guardiansData?.data.find((x) => x.id === gid) ??
                  ...
                  const guardian =
                    "name" in found
                      ? found
                      : (found as { guardian: Guardian }).guardian;
                  return (
                    <div
                      key={gid}
                      className="flex items-center gap-4 p-4 glass-panel rounded-xl bg-slate-900/30"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 truncate">
                          {[guardian.phone, guardian.email]
                            .filter(Boolean)
                            .join(" · ") || "Sem informações de contato"}
                        </p>
                      </div>
                      <Button
                        type="button"
        ...
        <GuardianPickerModal
          ...
          onSelect={(g) =>
            setValue("guardianIds", [...guardianIds, g.id], {
              shouldDirty: true,
            })
          }
