...
const classStatusVariant: Record<
  string,
...
export function StudentDetailPage() {
  ...
  if (!student)
    return <div className="text-muted-foreground">Aluno não encontrado</div>;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            {student.areasToImprove && (
              <div>
                ...
                <span>{student.areasToImprove}</span>
              </div>
            )}
            {student.goals && (
              <div>
      ...
      <Tabs defaultValue="evolution" variant="glass">
        ...
        <TabsPanel value="evolution" className="pt-4 space-y-3">
          ...
          {evolutionData && evolutionData.meta.totalPages > 1 && (
            <Pagination
              page={evolutionPage}
              totalPages={evolutionData.meta.totalPages}
              onPageChange={setEvolutionPage}
            />
          )}
        </TabsPanel>
            ...
            classData.data.map((cls) => (
              <div key={cls.id} className="glass rounded-xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant={classStatusVariant[cls.status] ?? "default"}
                    size="sm"
                  >
        ...
        {isGuardian && (
          <TabsPanel value="payments" className="pt-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="glass rounded-xl p-4">
                  ...
                  <p className="text-xl font-bold text-success">
                    {fmt(paymentData.meta.totalPaid)}
                  </p>
                </div>
                <div className="glass rounded-xl p-4">
                  <p className="text-xs text-muted-foreground">Pendente</p>
                  <p className="text-xl font-bold text-warning">
                    {fmt(paymentData.meta.totalPending)}
                  </p>
                </div>
              </div>
            )}
            ...
            {paymentData && paymentData.meta.totalPages > 1 && (
              <Pagination
                page={paymentPage}
                totalPages={paymentData.meta.totalPages}
